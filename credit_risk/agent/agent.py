"""
AI Agent for the IEEE Fraud Detection Pipeline
-----------------------------------------------
Uses a free local Ollama model (qwen2.5:7b by default) and connects
to mcp_server.py via SSE (HTTP) to run ML pipeline operations.

Prerequisites:
    1. Ollama running         →  ollama serve
    2. Model downloaded       →  ollama pull qwen2.5:7b
    3. MCP server running     →  python -m credit_risk.server.mcp_server (separate terminal)
    4. Packages installed     →  pip install "mcp[cli]" ollama

Usage:
    python -m credit_risk.agent.agent --query "What is the test AUC of the XGBoost model?"
    python -m credit_risk.agent.agent --query "Train XGBoost in fast mode" --ollama_model qwen2.5:7b
    python -m credit_risk.agent.agent --query "..." --mcp_url http://localhost:8000/sse
"""
...
SYSTEM_PROMPT = f"""\
You are an anomaly detection assistant for an IEEE credit risk project.
...
- Always include key numbers (ROC-AUC, anomaly %, confusion matrix) in your final answer.
- Available models: 'xgboost' (GPU, best accuracy) or 'rf' (CPU, random forest).
- Explain that recall is a better metric than precision for anomaly detection.
...
async def run_agent(query: str, ollama_model: str, data_dir: str, mcp_url: str) -> None:
    ...
    try:
        ...
        print('  source myenv/bin/activate && python -m credit_risk.server.mcp_server')
        sys.exit(1)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Anomaly Detection AI Agent — Ollama + MCP (free, local)'
    )
    parser.add_argument(
        '--query',
        type=str,
